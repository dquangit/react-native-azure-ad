...
export default class ReactNativeAD {
  ...
  saveCredentials(data: ADCredentials): Promise {
    ...
    const saveCridentials = data.map(resource => {
      ...
      return SecureStore.setItemAsync(
        _getResourceKey(this.config.client_id, resource),
        JSON.stringify(data[resource])
      );
    });

    Object.assign(this.credentials, data);
    return Promise.all(saveCridentials);
  }
            ...
            if (cred.response.access_token) {
              ...
              Promise.all([
                SecureStore.setItemAsync(
                  ...
                  cred.response.access_token
                ),
                SecureStore.setItemAsync(
                  `${cacheKey}refreshtoken`,
                  cred.response.refresh_token
                )
              ]).then(() => {
                resolve(cred)}).catch((error) => {
                });
            } else {
              log.debug(
...
function _getResourceKey(config: ADConfig, resourceId: string): string {
  const key = `${config.client_id}.${resourceId}`.split(/[\s,:/\-.]+/).join("");
  return key;
}
